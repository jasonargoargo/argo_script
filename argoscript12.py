...
def hconres(main):
    ...
    for x in hconres_BILLS_xmls:
        ...
        with open(x, 'r', encoding='utf8') as fulltext:
            ...
            try:
                db.BILLS_fulltext.update_one(data, {'$set': {'_id': data['_id'], 'fulltext': data['fulltext']}}, upsert=True)
            except pymongo.errors.ConnectionFailure as e:
                print(e)
                ...
                if elem.tag == 'committeeReports' or elem.tag == 'committeeReport' or elem.tag == 'citation':
                    if elem.tag == 'citation':
                        try:
                            ...
                            continue
                if elem.tag == 'subjects' or elem.tag == 'billSubjects' or elem.tag == 'policyArea' or elem.tag == 'name':
                    if elem.tag == 'name':
                        try:
                ...
                if elem.tag == 'subjects' or elem.tag == 'billSubjects' or elem.tag == 'legislativeSubjects' or elem.tag == 'item' or elem.tag == 'name':
                    if elem.tag == 'name':
                        try:
                            ...
                            continue
            try:
                db.BILLSTATUS_general_data.update_one(data, {'$set': {'_id': data['_id'], 'num': data['num'], 'title': data['title'], 'type': data['type'], 'sponsor_bioguideID': data['sponsor_bioguideID'], 'cosponsor_bioguideID': data['cosponsor_bioguideID'], 'cmte_reports': data['cmte_reports'], 'policy_areas': data['policy_areas'], 'legislative_subjects': data['legislative_subjects']}}, upsert=True)
            except pymongo.errors.ConnectionFailure as e:
                print(e)
        with open(x, 'r', encoding='utf8') as hconres_data:
